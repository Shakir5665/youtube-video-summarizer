# app.py - Professional UX Overhaul
import gradio as gr
...
copy_js = """
(text) => {
    if (!text || text.includes('loading-box') || text === "_Summary will appear here..._") return;
...
def summarize_video(url):
    ...
    yield "<div class='loading-box'><span class='loader'></span><br><b style='color:#a855f7;font-size:1.1rem;font-weight:600;'>Analyzing video & generating summary...</b></div>"
        ...
        try:
            ...
            try:
                ...
                if availability.get('languages'):
                    ...
                    yield f"📝 **No English transcript found**\n\n**Available languages:** \n{langs}\n\nTry a video with English captions."
                else:
                    yield f"❌ Could not extract transcript: {str(e2)[:200]}"
                return
                
        if len(transcript_text) < 100:
            yield "📹 **Video too short to summarize** (transcript under 100 characters)"
            return
...
def get_video_info(url):
    ...
    if availability.get('has_transcripts'):
        ...
        return f"❌ {availability.get('error', 'No transcript available')}", thumbnail_html
...
with gr.Blocks(title="YouTube Video Summarizer", theme=ultra_theme, css=custom_css, js=js_code) as app:
    ...
    with gr.Row(equal_height=True):
        ...
        with gr.Column(scale=7, elem_classes="glass-panel", elem_id="right-panel"):
            ...
            output_text = gr.Markdown("_Summary will appear here..._", elem_classes="summary-content prose")
